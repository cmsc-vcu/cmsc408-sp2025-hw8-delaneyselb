...
run_sql_and_return_html(cnx,"""
SELECT `Country Code`, `Short Name`, `country_abbr`, `country_wb_abbr`, `Region`
FROM wdi_country
...
FROM wdi_country
ORDER BY `Income Group`, `Short Name`;
""")
...
SELECT 
    ...
    SUM(CASE WHEN `Income Group` = 'Other' THEN 1 ELSE 0 END) AS `Other`,
...
run_sql_and_return_html(cnx,"""
SELECT 
    region_income_combinations.Region, 
    region_income_combinations.Income_Group
FROM 
    (SELECT DISTINCT Region FROM wdi_country) AS regions
CROSS JOIN 
    (SELECT DISTINCT `Income Group` FROM wdi_country) AS income_groups
LEFT JOIN 
...
ORDER BY region_income_combinations.Region, region_income_combinations.Income_Group;
""")
...
SELECT 
    region_income_combinations.Region, 
    region_income_combinations.Income_Group,
    COUNT(wdi_country.`Country Code`) AS country_count,
    SUM(COUNT(wdi_country.`Country Code`)) OVER (PARTITION BY region_income_combinations.Region) AS sum_countries_in_region,
    SUM(COUNT(wdi_country.`Country Code`)) OVER (PARTITION BY region_income_combinations.`Income Group`) AS sum_countries_by_income,
    COUNT(wdi_country.`Country Code`) OVER () AS total_sum_countries,
    (COUNT(wdi_country.`Country Code`) * 100.0) / COUNT(wdi_country.`Country Code`) OVER () AS percentage_of_total
FROM 
    (SELECT DISTINCT Region FROM wdi_country) AS region_income_combinations
CROSS JOIN 
    (SELECT DISTINCT `Income Group` FROM wdi_country) AS income_groups
LEFT JOIN 
    wdi_country
ON region_income_combinations.Region = wdi_country.Region
AND income_groups.`Income Group` = wdi_country.`Income Group`
GROUP BY 
    region_income_combinations.Region, 
    region_income_combinations.Income_Group
ORDER BY 
...
SELECT 
    Region,
    MAX(CASE WHEN Income_Group = 'High income' THEN percentage_of_total ELSE 0 END) AS "High income",
    MAX(CASE WHEN Income_Group = 'Upper middle income' THEN percentage_of_total ELSE 0 END) AS "Upper middle income",
    MAX(CASE WHEN Income_Group = 'Lower middle income' THEN percentage_of_total ELSE 0 END) AS "Lower middle income",
    MAX(CASE WHEN Income_Group = 'Low income' THEN percentage_of_total ELSE 0 END) AS "Low income",
    MAX(CASE WHEN Income_Group = 'Other' THEN percentage_of_total ELSE 0 END) AS "Other"
FROM 
    region_income_data
GROUP BY 
    Region
...
run_sql_and_return_html(cnx,"""
WITH income_category_data AS (
    SELECT 
        `Income Group`,
        COUNT(`Country Code`) AS country_count,
        COUNT(`Country Code`) OVER () AS total_sum_countries
    FROM 
        wdi_country
    GROUP BY 
        `Income Group`
...
SELECT 
    ...
    country_count,
    total_sum_countries,
    (country_count * 100.0) / total_sum_countries AS percentage_of_total
FROM 
